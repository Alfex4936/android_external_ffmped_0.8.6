...
#undef  fprintf
#define fprintf(P1,...) av_log(NULL,AV_LOG_ERROR,__VA_ARGS__)
#undef  puts
#define puts(...) 
#undef  perror
#define perror(...)
...
#define exit(...)
#undef  time
#define ffmpeg_exit(P1) ffmpeg_return(FFMPEG_EXIT_CODE_ERROR)
#endif

#ifdef __LGE__
int exit_code = FFMPEG_EXIT_CODE_NONE;
int64_t current_pts = 0LL; // in microsecond
#endif
...
#ifdef __LGE__
static int seek_start_time_keyframe = 0;
#endif
...
#endif
{
...
#ifdef __LGE__
	if (exit_code == FFMPEG_EXIT_CODE_NONE)
		exit_code = ret;

	av_log(NULL, AV_LOG_ERROR, "%s --> ret:%d, exit_code:%d, current_pts:%lld\n", __FUNCTION__, ret, exit_code, current_pts);
#endif

#ifdef __LGE__
	// free all resources at the last calling of ffmpeg_return
	if (ret == FFMPEG_EXIT_CODE_NORMAL) {
#endif
    ...
    for(i=0;i<nb_output_files;i++) {
		...
		if (s->priv_data) // check memory leak
		{
			av_log(NULL, AV_LOG_ERROR, "%s --> priv_data:%x should be freed \n", __FUNCTION__, s->priv_data);
			av_free(s->priv_data);
			s->priv_data = NULL;
		}
		else
		{
			av_log(NULL, AV_LOG_ERROR, "%s --> priv_data:%x is already null, maybe freed \n", __FUNCTION__, s->priv_data);
		}
#endif
...
#ifdef __LGE__
		exit_code = FFMPEG_EXIT_CODE_CANCEL;
		return FFMPEG_EXIT_CODE_CANCEL;
#else
        exit (255);
#endif
    }
...
#ifdef __LGE__ 
				//av_log(NULL, AV_LOG_ERROR, "%s:%d --> samples_size:%d, pkt->size:%d", __FUNCTION__, __LINE__, samples_size, (pkt)?pkt->size:0);
				// fix abort issue in (aacdec.c,ff_float_to_int16_interleave_neon)
				if (samples == NULL)
					samples_size = 0;
#endif
                     ...
                     AVStreamMap *stream_maps, int nb_stream_maps)
{
    ...
    for(i=0;i<nb_ostreams;i++) {
        ...
        if (ost->st->stream_copy) {
...
#ifdef __LGE__
                os->oformat->audio_codec = icodec->codec_id;
#endif
            ...
            case AVMEDIA_TYPE_VIDEO:
#ifdef __LGE__
                os->oformat->video_codec= icodec->codec_id;
#endif
...
#define METADATA_CHECK_INDEX(index, nb_elems, desc)\
 ...
 dump_format:
...
#ifdef __LGE__
    for(; received_sigterm == 0 && exit_code == FFMPEG_EXIT_CODE_NONE;) {
#else
    ...
    redo:
        ...
        {
#ifdef __LGE__
            av_log(NULL, AV_LOG_ERROR, "%s --> check limit_filesize:%lld, avio_tell returns %lld \n", __FUNCTION__, limit_filesize, avio_tell(output_files[0]->pb));
#endif
            break;
        }
...
#ifdef __LGE__
    av_log(NULL, AV_LOG_ERROR, "%s --> after flushing decoder buffers, avio_tell returns %lld \n", __FUNCTION__, avio_tell(output_files[0]->pb));
#endif
...
#ifdef __LGE__
    av_log(NULL, AV_LOG_ERROR, "%s --> after writing trailer, avio_tell returns %lld \n", __FUNCTION__, avio_tell(output_files[0]->pb));
#endif
...
#if defined(__LGE__) && defined(MALLOC_PREFIX)
	av_log(NULL, AV_LOG_ERROR, "%s --> priv_data:%x is allocated \n", __FUNCTION__, oc->priv_data);
#endif
...
#ifdef __LGE__
    { "seekstkf", OPT_BOOL | OPT_EXPERT, {(void*)&seek_start_time_keyframe}, "seek to keyframe just before start time" },
#endif
...
#endif
{
...
#ifdef __LGE__
	exit_code = FFMPEG_EXIT_CODE_NONE;
	received_sigterm = 0;
	q_pressed = 0;
	current_pts = 0LL;
	seek_start_time_keyframe = 0;
...
#ifdef __LGE__
	if (start_time > 0 && seek_start_time_keyframe) {
		int i;
		int64_t timestamp = start_time;
		for (i = 0; i < nb_input_files; i++) {
			av_log(NULL, AV_LOG_ERROR, "input_files[%d]: seek to position %0.3f\n",
					i, (double)timestamp / AV_TIME_BASE);
			int ret = av_seek_frame(input_files[i].ctx, -1, timestamp, AVSEEK_FLAG_BACKWARD);
			if (ret < 0) {
				fprintf(stderr, "input_files[%d]: could not seek to position %0.3f\n",
						i, (double)timestamp / AV_TIME_BASE);
				ffmpeg_exit(1);
				break;
			}
	...
	if (exit_code != FFMPEG_EXIT_CODE_NONE)
	{
		av_log(NULL, AV_LOG_ERROR, "exit code is already set to %d, skip to call transcode function \n", exit_code);
		goto L_FFMPEG_MAIN_RETURN;
	}
#endif
...
#ifdef __LGE__
L_FFMPEG_MAIN_RETURN:
#endif

#ifdef __LGE__
	return ffmpeg_return(FFMPEG_EXIT_CODE_NORMAL);
#else
