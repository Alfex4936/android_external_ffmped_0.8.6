...
int av_seek_frame(AVFormatContext *s, int stream_index, int64_t timestamp, int flags)
{
    ...
    if (ret >= 0) {
#ifdef __LGE__
        //av_log(NULL, AV_LOG_ERROR, "av_seek_frame --> s->iformat->read_seek is called, iformat->long_name:%s, ret(seek time):%d\n", s->iformat->long_name, ret);
        return ret;
#endif
        return 0;
    }
...
#if defined(__LGE__) && defined(__LGE_FIX_MEMLEAK_AVFORMAT_PRIV_DATA__)
        if (s->priv_data)
        {
            av_log(NULL, AV_LOG_ERROR, "%s --> free priv_data:0x%x to fix memory leakage \n", __FUNCTION__, s->priv_data);
            av_freep(&s->priv_data);
        }
#endif
...
#ifdef __LGE__
    av_log(NULL, AV_LOG_ERROR, "avformat_write_header --> s->oformat->name:%s, audio_codec:%d, video_codec:%d\n", \
		s->oformat->name, s->oformat->audio_codec, s->oformat->video_codec);
#endif
