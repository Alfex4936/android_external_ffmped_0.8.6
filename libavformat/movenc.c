...
#ifdef __LGE__
    // trim mp4 format with h263 codec
    if (track->enc->codec_id == CODEC_ID_H263)
    {
        return MKTAG('s','2','6','3');
    }
    // trim mp4 format with amrnb codec
    if (track->enc->codec_id == CODEC_ID_AMR_NB)
    {
        return MKTAG('s','a','m','r');
    }
#endif
...
#ifdef __LGE__
    av_log(NULL, AV_LOG_ERROR, "mov_find_codec_tag --> track->mode:%d, track->enc->codec_id:%d\n", track->mode, track->enc->codec_id); 
#endif
...
static int mov_write_tkhd_tag(AVIOContext *pb, MOVTrack *track, AVStream *st)
{
    ...
    /* Matrix structure */
#ifdef __LGE__
    av_log(NULL, AV_LOG_ERROR, "mov_write_tkhd_tag --> track->rotation:%d\n", track->rotation);
    if(track->rotation == 90) {
        avio_wb32(pb, 0x0); /* reserved [0][0]*/
        avio_wb32(pb, 0x00010000); /* reserved [0][1]*/
        avio_wb32(pb, 0x0); /* reserved --*/
        avio_wb32(pb, 0xffff0000); /* reserved [1][0]*/
        avio_wb32(pb, 0x0); /* reserved [1][1] */
        avio_wb32(pb, 0x0); /* reserved  --*/
        avio_wb32(pb, 0x0); /* reserved [2][0]*/
        avio_wb32(pb, 0x0); /* reserved [2][1]*/
        avio_wb32(pb, 0x40000000); /* reserved --*/        
    } else
    if(track->rotation == 180) {
        avio_wb32(pb, 0xffff0000); /* reserved [0][0]*/
        avio_wb32(pb, 0x0); /* reserved [0][1]*/
        avio_wb32(pb, 0x0); /* reserved --*/
        avio_wb32(pb, 0x0); /* reserved [1][0]*/
        avio_wb32(pb, 0xffff0000); /* reserved [1][1] */
        avio_wb32(pb, 0x0); /* reserved  --*/
        avio_wb32(pb, 0x0); /* reserved [2][0]*/
        avio_wb32(pb, 0x0); /* reserved [2][1]*/
        avio_wb32(pb, 0x40000000); /* reserved --*/        
    } else
    if(track->rotation == 270) {
        avio_wb32(pb, 0x0); /* reserved [0][0]*/
        avio_wb32(pb, 0xffff0000); /* reserved [0][1]*/
        avio_wb32(pb, 0x0); /* reserved --*/
        avio_wb32(pb, 0x00010000); /* reserved [1][0]*/
        avio_wb32(pb, 0x0); /* reserved [1][1] */
        avio_wb32(pb, 0x0); /* reserved  --*/
        avio_wb32(pb, 0x0); /* reserved [2][0]*/
        avio_wb32(pb, 0x0); /* reserved [2][1]*/
...
#if defined(__LGE__) && defined(__LGE_TAG_UDTA_AUTH__)
static int mov_write_udta_auth_lge_tag(AVIOContext *pb, MOVMuxContext *mov, AVFormatContext *s)
{
	int64_t pos, pos2;
	...
	pos2 = avio_tell(pb);
	avio_wb32(pb, 0); /* size */
	ffio_wfourcc(pb, "auth");
	avio_wb32(pb, 0); // version + flag
	avio_wb16(pb, 0x15c7);
	avio_w8(pb, 'L');
	avio_w8(pb, 'G');
	avio_w8(pb, 'E');
                              ...
                              AVFormatContext *s)
{
	...
	{
		int is_lge_video = 0;
		AVDictionaryEntry *udta_auth = av_dict_get(s->metadata, "auth", NULL, 0);
		if (udta_auth != NULL) {
			if (udta_auth->value != NULL) {
				if (strncmp(udta_auth->value, "LGE", strlen("LGE")) == 0) {
					is_lge_video = 1;
				}
			}
		}
		av_log(NULL, AV_LOG_ERROR, "%s --> from udta_auth, is_lge_video:%d\n", __FUNCTION__, is_lge_video);
		if (is_lge_video)
			mov_write_udta_auth_lge_tag(pb, mov, s);
	}
#endif
...
#if defined(__LGE__) && defined(__LGE_TAG_UDTA_CPRT__)
static int mov_write_udta_cprt_lge_tag(AVIOContext *pb, MOVMuxContext *mov, AVFormatContext *s)
{
	int64_t pos, pos2;
...
#if defined(__LGE__) && defined(__LGE_TAG_FTYP__)
	{
		int is_lge_video = 0;
		AVDictionaryEntry *minor_version = av_dict_get(s->metadata, "minor_version", NULL, 0);
		if (minor_version != NULL) {
			char minor_ver_str[11]; /* 32 bit integer -> 10 digits + null */
			memset(&minor_ver_str[0], 0x00, sizeof(minor_ver_str));
			snprintf(minor_ver_str, sizeof(minor_ver_str), "%d", MKBETAG('L','G','E',' '));
			if (minor_version->value != NULL) {
				if (strncmp(minor_version->value, minor_ver_str, strlen(minor_ver_str)) == 0) {
					is_lge_video = 1;
				}
			}
		}
		av_log(NULL, AV_LOG_ERROR, "%s --> from minor_version, is_lge_video:%d\n", __FUNCTION__, is_lge_video);
		if (is_lge_video)
			ffio_wfourcc(pb, "LGE ");
		else
			avio_wb32(pb, minor);
...
#ifdef __LGE__
            AVDictionaryEntry *rotation = av_dict_get(st->metadata, "rotate", NULL, 0);
            if(rotation != NULL) {
                track->rotation = atoi(rotation->value);
                av_log(NULL, AV_LOG_ERROR, "mov_write_header --> key:%s, value:%s, track->rotation:%d\n", rotation->key, rotation->value, track->rotation);
            }
            else {
                track->rotation = 0;
                av_log(NULL, AV_LOG_ERROR, "mov_write_header --> rotation metadata is NULL\n");
            }
#endif
...
static int mov_write_trailer(AVFormatContext *s)
{
	...
	{
		int is_lge_video = 0;
		AVDictionaryEntry *udta_cprt = av_dict_get(s->metadata, "cprt", NULL, 0);
		if (udta_cprt != NULL) {
			if (udta_cprt->value != NULL) {
				if (strncmp(udta_cprt->value, "LGE ", strlen("LGE ")) == 0) {
					is_lge_video = 1;
				}
			}
		}
		av_log(NULL, AV_LOG_ERROR, "%s --> from udta_cprt, is_lge_video:%d\n", __FUNCTION__, is_lge_video);
		if (is_lge_video)
			mov_write_udta_cprt_lge_tag(pb, mov, s);
	}
#endif
...
#ifdef __LGE__
AVOutputFormat ff_3gpp_muxer = {
    "3gpp",
    NULL_IF_CONFIG_SMALL("3GPP format"),
    NULL,
    "3gp",
    sizeof(MOVMuxContext),
    CODEC_ID_AMR_NB,
