...
static int mov_read_default(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    while (total_size + 8 < atom.size && !url_feof(pb)) {
		...
		if (atom.type == MKTAG('u','d','t','a'))
		{
			if (a.type == MKTAG('c','p','r','t') && a.size >= 8)
			{
				// {0x0, 0x0, 0x0, 0x0, 'L', 'G', 'E', ' ', 0x0, 0x0, 0x0, 0x0}
				av_log(NULL, AV_LOG_ERROR, "%s --> find udta/cprt, data:'%c%c%c%c'\n", \
						__FUNCTION__,
						(pb->buf_ptr[4]),
						(pb->buf_ptr[5]),
						(pb->buf_ptr[6]),
						(pb->buf_ptr[7]));
				int copyright = *(((int*)pb->buf_ptr) + 1);
				if (copyright == MKTAG('L','G','E',' '))
				{
					av_dict_set(&c->fc->metadata, "cprt", "LGE ", 0);
					parse = NULL;
				}
			}
		}
		...
		if (atom.type == MKTAG('u','d','t','a'))
		{
			if (a.type == MKTAG('a','u','t','h') && a.size >= (4+2+3))
			{
				// {0x0, 0x0, 0x0, 0x0, 0x15, 0xc7, 'L', 'G', 'E'}
				av_log(NULL, AV_LOG_ERROR, "%s --> find udta/auth, data:'%c%c%c'\n", \
						__FUNCTION__,
						(pb->buf_ptr[6]),
						(pb->buf_ptr[7]),
						(pb->buf_ptr[8]));
				if ((pb->buf_ptr[6] == 'L') &&
					...
					(pb->buf_ptr[8] == 'E') )
				{
					av_dict_set(&c->fc->metadata, "auth", "LGE", 0);
					parse = NULL;
				}
			}
		}
...
#ifdef __LGE__
    if (display_matrix[0][1] == 65536 && display_matrix[1][0] == -65536)
...
#ifdef __LGE__
    {
        int real_time = av_rescale(seek_timestamp, s->streams[stream_index]->time_base.num*1000, s->streams[stream_index]->time_base.den);
        //av_log(NULL, AV_LOG_ERROR, "mov_read_seek --> real_time:%d\n", real_time);
        return real_time;
    }
#endif
    return 0;
