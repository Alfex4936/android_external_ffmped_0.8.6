...
#if defined(__LGE__) && defined(__LGE_FIX_MEMLEAK_AVFILTER_PICTURE_POOL__)
...
//        av_freep(&(*link)->pool);
		...
		if ((*link)->pool)
		{
			av_log(NULL, AV_LOG_ERROR, "%s --> why doesn't free (*link)->pool:0x%x \n", __FUNCTION__, (*link)->pool);
			av_log(NULL, AV_LOG_ERROR, "%s --> double free hotfix, this fix introduces a small memleak \n", __FUNCTION__);
		}
#endif
    }
#endif
